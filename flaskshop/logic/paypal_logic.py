# coding: utf-8
"""Logic for interacting with Paypal."""
...
class PayPalForm(object):
    ...
    def __init__(self, transaction, currency='GBP', data=None, form_attr=None,
        ...
        # setting values required by paypal
    ...
    def as_fields(self):
        ...
        all_fields = {
            'merchant_id': APP.config['PAYPAL_MERCHANT_ID']
        }
        all_fields['merchant_response_url'] = APP.config['PAYPAL_RESPONSE_URL']
    ...
    def is_valid(self):
        """Validates the response from PAYPAL. Raises an exception if
...
def process_payment(request,order_id,hash,paypal_id):
    ...
    if True:#paypal_transaction.status[0]:
        ...
        APP.log_manager.log_event(
            'Failed Card Payment. Error is {0}.'.format(paypal_transaction.status[0]
            ),
            tickets=transaction.tickets,
            user=transaction.user,
            transaction=transaction,
