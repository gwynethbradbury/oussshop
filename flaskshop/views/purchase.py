...
from flaskshop.helpers import validators
...
from flaskshop.logic import paypal_logic
from flaskshop.logic import purchase_logic
from flaskshop.logic import payment_logic
# from flaskshop.logic.custom_logic import ticket_logic
from flaskshop.views import shop
# APP = app.APP#DB = db.DB
APP = flask.current_app
from flaskshop.app import oussshopdb as DB

from flaskshop import dbconfig
...
def purchase_home():
    ...
    loggedIn, firstName, noOfItems = shop.getLoginDetails()
    ...
    numobject = {}
    for i in items:
        objects.append(models.Product.query.get_or_404(i.product_id))
        if models.Product.query.get_or_404(i.product_id).name in numobject:
            numobject[models.Product.query.get_or_404(i.product_id).name] += 1
        else:
    ...
    totalObjects=0
    for p in products:
        totalPrice += p.price*numobject[p.name]
        totalObjects += numobject[p.name]
    totalPrice = '{0:03d}'.format(totalPrice)
    totalPrice = totalPrice[:-2] + '.' + totalPrice[-2:]



    if flask.request.method == 'POST':

        flashes = purchase_logic.validate_objects(
            products,
            numobject
        )
    ...
    #         )
        ...
        # objects = purchase_logic.create_objects(
        #     login.current_user,
        #     ticket_info,
        #     num_tickets
        # )
